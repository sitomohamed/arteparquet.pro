...
const schema = z.object({
  projectType: z.string().min(1).max(50).regex(/^[a-z-]+$/),
  clientType: z.string().min(1).max(50).regex(/^[a-z-]+$/),
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // 5. Schema validation with enhanced checks
    const rawData = schema.parse(body)
    
    // 6. Additional input sanitization
    const data = {
      ...rawData,
      name: sanitizeInputServer(rawData.name),
      city: sanitizeInputServer(rawData.city),
    ...
    let errorMessage = 'Errore nell\'invio. Riprova o contattaci su WhatsApp.'
    let statusCode = 500
    
    if (err instanceof z.ZodError) {
      errorMessage = 'Dati del modulo non validi'
      statusCode = 400
    } else if (err instanceof Error) {
      // Only expose safe error messages
      if (err.message.includes('Invalid email') || err.message.includes('Invalid phone')) {
        errorMessage = 'Email o telefono non validi'
        statusCode = 400
      }
    }
