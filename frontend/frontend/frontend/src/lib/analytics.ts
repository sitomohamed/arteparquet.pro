...
/**
 * Track photo upload interactions (primary conversion asset)
 */
export function trackPhotoUploadStart() {
  trackEvent('photo_upload_start', { form_name: 'photo_form' })
}

export function trackPhotoUploadComplete(photoCount: number) {
  trackEvent('photo_upload_complete', {
    form_name: 'photo_form',
  ...
  trackEvent('generate_lead', {
    value: 1,
    currency: 'EUR',
    lead_type: 'photo_upload',
  })
}

/**
 * Track UTM parameters on first visit (for attribution)
 */
export function trackUTMParameters() {
  ...
  if (utm.source) {
    trackEvent('utm_session', {
      utm_source:   utm.source,
      utm_medium:   utm.medium   ?? '',
      utm_campaign: utm.campaign ?? '',
    })
    // Store in sessionStorage for use in form submissions
    sessionStorage.setItem('arteparquet_utm', JSON.stringify(utm))
  }
}

/**
 * Get stored UTM data for enriching form submissions
 */
export function getStoredUTM(): Record<string, string> {
  if (typeof window === 'undefined') return {}
  try {
    return JSON.parse(sessionStorage.getItem('arteparquet_utm') ?? '{}')
  } catch {
    return {}
  }
}
